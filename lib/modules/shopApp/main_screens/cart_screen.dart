...
import '../../../model/shop_models/cart/cart_broduct_model.dart';
      ...
      builder: (context, state) {
        return  Scaffold(
          body:ConditionalBuilder(
            condition: ShopCubit.get(context).cart != null,
              ...
              children: [
                Container(
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      ...
                      mainAxisAlignment: MainAxisAlignment.start,
                      children: [
                        Text('Total Cost : ',style: TextStyle(color: Colors.white,fontSize: 8),),
                        SizedBox(width: 1,),
                        Text(' ${ShopCubit.get(context).cart!.data!.total} \$',style: Theme.of(context).textTheme.bodySmall!.copyWith(color: Colors.white,fontSize: 8,height: 0),),
                      ],
                    ),
                  ),
                ),
                Expanded(
                  child: ListView.separated(
                      itemBuilder: (context, index) => CartProductsItemBuilder(ShopCubit.get(context).cart!.data!.cart_items![index].product,context,index),
                      separatorBuilder: (context, index) => Container(height: 1,width: double.infinity,color: Colors.grey,),
                      itemCount: ShopCubit.get(context).cart!.data!.cart_items!.length
                  ),
                ),
              ],
            ),
...
Widget CartProductsItemBuilder(InCartProductModel? model,context,index)=>InkWell(
  onTap: () {
  },
  child:   Padding(
